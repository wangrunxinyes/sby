...
$point_a = new point ( $location->a, $location->b, $location->x, $location->y );
...
foreach ( $type as $key => $value ) {
	...
	while ( $i < 4 ) {
		...
		while ( $j < 4 ) {
			...
			$bounds = $point_a->a . "." . $point_a->x . "," . $point_a->b . "." . $point_a->y;
			// . ","
			// . $point_b->a .".". $point_b->x . ","
			// . $point_b->b .".". $point_b->y;
			...
			$request = "http://" . $_SERVER ['HTTP_HOST'] . Yii::app ()->assets->getUrlPath ( "/wang/poi/bounds/" . $bounds . "/category/" . $category . "/store/" . $store_category );
