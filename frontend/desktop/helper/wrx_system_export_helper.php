...
class wrx_system_export_helper {
	...
	public function create($type, $period) {
		...
		switch ($type) {
			case 'weixin_msg' :
				$this->title = "用户信息";
				$sql = 'select msg.keyword, msg.time, msg.reply, user.weixin_name from weixin_message_db msg left join ' . 'weixin_user_db user on msg.fromusername = user.user_id ' . 'where msg.time > "' . $period ['s'] . '" ' . 'AND msg.time < "' . $period ['e'] . '" ';
			...
			case 'post_msg_history' :
				$this->title = "微信推送记录";
				$sql = 'select re.*, msg.title from weixin_post_msg_record_db re left join ' . 'weixin_post_msg_db msg on re.main_msg_id = msg.id ' . 'where post_time > "' . $period ['s'] . '" ' . 'AND post_time < "' . $period ['e'] . '" ';
				$unset_arr = array (
						'id',
						'post_by',
						'details',
		...
		$data ['num'] = $num [0] ['num'];
		$this->des = "记录开始时间：" . $period ['read-s'] . ", 结束时间：" . $period ['read-e'] . ", 数据共" . $data ['num'] . "条。";
