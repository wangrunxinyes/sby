...
class processer_product_analysis {
	...
	function count_behavior() {
		...
		$sql = 'select ref_key from wrx_model_behavior where behavior_type = "product" AND target_id = "' . Yii::app ()->weixin->getWeixin ( 'weixin_app_key' ) . '" AND time > "' . $time . '"';
